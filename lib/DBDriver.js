...
DBDriver.prototype._cancelPendings = function(error){
  for (var cSlotID = 0, len = this.requests.length; cSlotID < len; cSlotID++) {
    var req = this.requests[cSlotID];
    if (req) {
      // may emulate a error frame
      // me.requests[cSlotID].emit('frame', null, cSlotID, C.ERROR_FRAME, 0, (new buffer(error)).length, error);
      req.emit('error', error);
      delete this.requests[cSlotID];
      if (--this.execCount === 0) return; // must no more pending requests
    }
  }
};
