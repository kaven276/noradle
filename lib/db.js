...
function Slot(c, slotID, oraSid, oraSerial){
  ...
  this.bindSock(c, slotID, oraSid, oraSerial);
  // properties below has value only when
  this.bTime = undefined;
  this.env = undefined;
  this.response = undefined;
}
Slot.prototype.bindSock = function(oraSock, slotID, oraSid, oraSerial){
...
Slot.prototype.releaseSock = function(){
...
var dbListener = net.createServer({allowHalfOpen : true}, function(c){
  ...
  var slot, slotID, oraSid, oraSerial, dbInst, dbNamesLen, dbNames;
  var dbName, dbDomain, dbUniqueName, dbRole;
  ...
  function onHandshake(data){
    ...
    dbInst = data.readInt32BE(16);
    dbNamesLen = data.readInt32BE(20);
    dbNames = data.slice(24).toString().split('/');
    dbName = dbNames[0];
    dbDomain = dbNames[1];
    dbUniqueName = dbNames[2];
    dbRole = dbNames[3];
  ...
  function init(){
    ...
    if (slot) {
      ...
      slot.bindSock(c, slotID, oraSid, oraSerial);
    } else {
      slot = dbPool[slotID] = new Slot(c, slotID, oraSid, oraSerial);
    }
    ...
    c.on('timeout', function(){
      ...
      slot.releaseSock(); // todo: if socket is in use, it may cause bug
      c.end();
    });
...
function WaitRec(env, cliSock, cb){
  ...
  this.cb = cb;
  this.date = Date.now();
  cliSock.on('error', function(){
    logger.db('cliSock error');
  });
}
...
setInterval(function(){
  ...
  for (var i = waitQueue.length - 1; i >= 0; i--) {
    ...
    if (now - item.date > cfg.FreeConnTimeout) {
      ...
      item.cb(new Error('timeout')); // null parameter stand for timeout
