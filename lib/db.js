...
var DBListener = net.createServer(function(c) {
    ...
    c.on('end',
    function() {
        console.log('oracle server disconnected ' + DBListener.connections);
        db.del(c);
    });
    c.on('error',
    function() {
        console.log('oracle server connection has errors ' + DBListener.connections);
    });
    c.setKeepAlive(true, 1000 * 60);
});
DBListener.listen(cfg.oracle_port,
function() {
