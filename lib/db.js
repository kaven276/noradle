...
function prnFreelist(){
  var seqs = freeList.map(function(item){
    return item.seq;
  });
  logger.db(seqs);
}
...
var dbListener = net.createServer(function(c){
  process.nextTick(function(){
    dbev.emit('connect', dbListener.connections);
  });
...
function reportProtocolError(oraSock){
  ...
  oraSock.end();
  for (var i = 0, len = busyList.length; i < len; i++) {
    if (busyList[i].oraSock === oraSock) {
      busyList.splice(i, 1);
      return;
    }
  }
...
setInterval(function(){
  ...
  busyList.forEach(function(item){
    ...
    if (now - item.date > 3000) {
      ...
      // todo: execute longer than 3s, may do alert
    }
  });
}, 3000);
...
exports.connectionMonitor = dbev;
