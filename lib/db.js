 ...
 */
function DB(data){
  var dbNamesLen = data.readInt32BE(24)
    , dbNames = data.slice(28).toString().split('/')
    ;
  this.instance = data.readInt32BE(20);
  this.name = dbNames[0];
...
function Slot(c, slotID, oraSid, oraSerial, oraSpid, db){
  ...
  this.bindSock(c, slotID, oraSid, oraSerial, oraSpid, db);
  // properties below has value only when
  this.bTime = undefined;
  this.env = undefined;
  this.response = undefined;
}
Slot.prototype.bindSock = function(oraSock, slotID, oraSid, oraSerial, oraSpid, db){
  this.oraSock = oraSock;
  this.slotID = slotID;
  this.oraSid = oraSid;
  this.oraSerial = oraSerial;
  this.oraSpid = oraSpid;
  this.db = db;
  this.status = FREE;
  logRegular && this.log();
  if (this.sockCount === 0) {
...
var dbListener = net.createServer({allowHalfOpen : true}, function(c){
  ...
  var slot, slotID, oraSid, oraSerial, oraSpid, db;
  ...
  function onHandshake(data){
    ...
    oraSpid = data.readInt32BE(12)
    slotID = data.readInt32BE(16);
    db = new DB(data);
  ...
  function init(){
    ...
    if (slot) {
      ...
      slot.bindSock(c, slotID, oraSid, oraSerial, oraSpid, db);
    } else {
      slot = dbPool[slotID] = new Slot(c, slotID, oraSid, oraSerial, oraSpid, db);
    }
