...
Slot.prototype.listenQuit = function(undo){
  var slot = this;
  if (undo) {
    slot.oraSock.removeListener('readable', listenQuit);
  } else {
    slot.oraSock.once('readable', listenQuit);
  }
  function listenQuit(){
    if (slot.status === FREE) {
      slot.quit();
      slot.log();
      logger.db(' got qutting signal on free sts!');
      slot.oraSock.read();
    } else {
      slot.log();
      console.error(' got qutting signal on not free sts!', slot.status);
    }
  }
};
...
var dbListener = net.createServer({allowHalfOpen : true}, function(c){
  ...
  function onHandshake(){
    var data = c.read();
    if (data.slice(-1).readUInt8(0) !== 0) {
      logger.db('partial oracle connect data', data);
      c.unshift(data);
      return;
    }
    ...
    oraSockAttrSet = new OraSockAttrSet(data);
