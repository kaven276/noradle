...
var stat = {allCnt : 0, reqCnt : 0, cssCnt : 0, fbCnt : 0}
  ...
  , stsNames = ',FREE,BUSY,FREEING,CLOSED'.split(',')
  ;
...
exports.showStatus = function(req, res){
  ...
  printTR([ 'slotID', 'oraSid', 'oraSerial', 'status',
    'sockCount', 'reqCount', 'time ms(sum)', 'time ms(avg)',
    'bytesRead(this)', 'bytesRead(total)',
    'bytesWritten(this)', 'bytesWritten(total)' ]);
  db.dbPool.forEach(function(rec, i){
    ...
    /*w(fmt('NO.%d (%d:%d) %s sockets(%d),requests(%d) reads(%d,%d) write(%d,%d)',
     rec.slotID, rec.oraSid, rec.oraSerial, stsNames[rec.status],
     rec.sockCount, rec.reqCount,
     c.bytesRead, c.bytesRead + rec.hBytesRead,
     c.bytesWritten, c.bytesWritten + rec.hBytesWritten
     ));*/
    printTR([ rec.slotID, rec.oraSid, rec.oraSerial, stsNames[rec.status],
