  ...
  function connect(){
    ...
    function one(){
      ...
      cliSock.on('connect', function(c){
        var sts = dbPool.onConnectForDBPool(c);
        actualAmount++;
        if (sts === 0) {
          retryDelay = 0;
        }
      });
      cliSock.on('end', function(c){
        actualAmount--;
        if (retryDelay) {
          setTimeout(one, retryDelay);
          if (retryDelay < 64) {
            retryDelay *= 2;
          }
        } else {
...
DBPool.prototype.onConnectForDBPool = function(c){
  var me = this
    , slots = me.slots
