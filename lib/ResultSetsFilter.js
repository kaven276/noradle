...
module.exports = function ResultSetsFilter(oraRes, ohdr, flags){
  console.log('in ResultSetsFilter', ohdr['Content-Type']);
  if (ohdr['Content-Type'].substr(0, mimeLen) === mimeType) ; else {
    return oraRes;
  }
  var chunks = []
    , count = 0
  ...
  oraRes.on('end', function(){
    ...
    // ohdr['Content-Type'] = 'text/plain';
    ohdr['Content-Length'] = (bJson.length).toString();
    console.log('in ResultSetsFilter', bJson.length, rJson, ohdr);
    ee.emit('end', [bJson]);
  });
