...
exports.server4oracle = net.createServer(function(c){
  ...
  function onHandshake(){
    ...
    debug('oracle(%s) slot add, freeList=%j', slotID, freeList);
  }

  c.on('end', function(){
    debug('oracle(%d) disconnected', slotID);
    // find free list and remove from free list
    delete oraPool[slotID];
    var pos = freeList.indexOf(slotID);
    if (pos >= 0) {
      freeList.splice(pos, 1);
      debug('oracle(%d) slot removed, freeList=%j', slotID, freeList);
    } else {
      debug('oracle(%d) slot not in freeList.freeList(%j)', slotID, freeList);
