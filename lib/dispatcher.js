  ...
  c.on('end', function(){
    ...
    if (pos >= 0) {
      ...
      var oSlot = oraSessions[oSlotID]
        , client = clients[oSlot.cSeq]
        ;
      if (client && oSlot.cTime === client.cTime) {
        debug('busy oSlot(%s) slot socket end, cSeq(%s,%d), cSlotid(%d)', oSlotID, client.cid, oSlot.cSeq, oSlot.cSlotID);
        frame.writeFrame(client.socket, oSlot.cSlotID, C.ERROR_FRAME, 0, 'oracle connection break!');
        frame.writeFrame(client.socket, oSlot.cSlotID, C.END_FRAME, 0);
      }
    }
