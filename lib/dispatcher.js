...
function bindOSlot(req, cSeq, cSlotID, cSock, oSlotID){
  req.oSlotID = oSlotID;
...
function afterNewAvailableOSlot(oSlotID, isNew){
  ...
  if (w) {
    ...
    delete req.buf;
    bindOSlot(req, cSeq, cSlotID, clients[cSeq].socket, oSlotID);
    req.sendTime = Date.now();
...
function serveOracle(c){
  ...
  c.on('end', function(){
    ...
    if (pos >= 0) {
      ...
      if (!oSlot.quitting) {
        debug('oracle seq(%s) oSlot(%s) slot not in freeList.freeListCount(%d) cSlotid(%d)', connSeq, oSlotID, freeOraSlotIDs.length, oSlot.cSlotID);
        // todo: frame.writeFrame(clients[oSlot.cSeq].socket, oSlot.cSlotID, C.ERROR_FRAME, 0);
      }
    }
  ...
  getClients : function(cb){
    ...
    for (var i = 0; i < clientsHW; i++) {
      ...
      if (client) {
        ...
        _.each(client.cSlots.slice(0, client.cur_concurrency), function(cSlot, cSlotID){
          if (!cSlot) return;
