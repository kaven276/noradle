        ...
        if (req.method === 'POST') {
            if (req.headers['content-type'] === 'application/x-www-form-urlencoded') {
                ...
                function(field, file) {
                    ...
                    if (file.name === '') return;
                    if (dirs[field]) {
                        console.log('dirs[field]=' + dirs[field]);
                        var rpath = dirs[field] + (dirs[field].substr( - 1) === '/' ? file.name: '');
                        util.ensureDir(rpath);
                        file.path = cfg.upload_dir + rpath;
                    }
                    console.log(file.path);
                    fields.push(field, file.path.substr(uploadTrim));
                })
                .on('end',
                function() {
                    console.log('-> upload done');
                    oraSock.write(fields.join(CRLF));
                    oraSock.write(CRLF3);
                });
                form.parse(req);
                ...
                oraSock.write(CRLF2);
                console.error(url + ' has content-type unknown ' + req.headers['content-type']);
                req.on('data',
                function(chunk) {
