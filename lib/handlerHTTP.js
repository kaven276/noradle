  ...
  return function httpGatewayHandler(req, res, next){
    ...
    (function(){
      ...
      function onResponse(oraRes){
        ...
        var type = (rb.x$pack || rb.x$proc).substr(-1)
          ;
        ...
        // note: error response means rollback, will not cause repeated write
          if (oraRes.headers['Content-Length'] === '0') {
        if (type === 'c' && res.statusCode === 200 && !req.headers['x-requested-with']) {
          ...
          } else if (oraRes.headers['Content-Type'].match(/^text\/html;/)) {
            // may streamed or not
        ...
        // after this, all filter can only add/modify/delete ohdr properties
        mergeHeaders(ohdr, oraRes.headers);

        // step 3: content converters
        oraRes = ResultSetsFilter(oraRes, ohdr, flags, res);

        // oraRes = MD5CalcFilter(oraRes, ohdr, flags);

        // step 5: zip transfer
        oraRes = zipFilter(oraRes, req, ohdr, {zip_threshold : cfg.zip_threshold});


        oraRes.on('data', function(data){
          if (!res.headersSent) {
            res.writeHead(res.statusCode, ohdr);
          }
          res.write(data);
        });
