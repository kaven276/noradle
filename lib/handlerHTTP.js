  ...
  function pspdweb(req, res, next){
    ...
    (function(){
      ...
      function onResponse(oraRes){
        ...
        var cLen = parseInt(oraRes.headers['Content-Length'])
        ...
        debug('type=%s, status=%d, cLen=%d', type, oraRes.status, cLen);
        if (type === 'c' && oraRes.status === 200 && !req.headers['x-requested-with']) {
          if (cLen === 0) {
        ...
        flags.headFixed && res.writeHead(oraRes.status, ohdr);
