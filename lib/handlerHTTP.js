    ...
    (function(){
      ...
      function sendAll(){
        var interrupter = dbPool.findFree(reqUrl, null, function(err, oraReq){
          ...
          res.setHeader('x-pw-time-node-sent', Date.now() - sTime);
        });
      }
      ...
      function onResponse(oraRes){
        res.setHeader('x-pw-time-node-got-head', Date.now() - sTime);
        ...
        oraRes.on('data', function(data){
          res.headersSent || res.setHeader('x-pw-time-node-write-head', Date.now() - sTime);
          res.write(data);
        });
