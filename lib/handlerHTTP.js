...
module.exports = function(dbPool, ReqBaseC, customCfg){
  ...
  return function httpGatewayHandler(req, res, next){
    ...
    if (urlParse(reqUrl).pathname === '/_about') {
    ...
    (function(){
      ...
      if (GatewayCache) {
        var oraRes = GatewayCache.beforeFilter(req, rb);
      ...
      function sendAll(){
        var interrupter = dbPool.findFree(reqUrl, null, function(err, oraReq){
          ...
          rb.t$timespan && res.setHeader('x-pw-ntime-1-sent', Date.now() - sTime);
        });
      }
      ...
      function onResponse(oraRes){
        rb.t$timespan && res.setHeader('x-pw-ntime-2-got-head', Date.now() - sTime);

        oraRes = GatewayCache.afterFilter(oraRes, res, req, rb);
        ...
        if (feedbackFilter(oraRes, res, req)) {
          return;
        }

        // step 4: content converters
        oraRes = resultSetsFilter(oraRes, res);
        ...
        oraRes.on('data', function(data){
          rb.t$timespan && (res.headersSent || res.setHeader('x-pw-ntime-3-write-head', Date.now() - sTime));
          res.write(data);
        });
