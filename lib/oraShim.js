  ...
  var c = net.connect(8000, function(){
    ...
    // send db attrs to nodejs on the head of oraSock
    {
      ...
      var dbNames = ['dialbook', 'noradle.com', dbUniqueName, 'PRIMARY'].join('/')
        , data = new Buffer(28 + dbNames.length)
        ;
      // magicNum, sid, serial, spid, slot, inst, rest length
      writeStruct(data, [197610261, i, 0, i, i, -1, dbNames.length]);
      data.write(dbNames, 28, dbNames.length);
      c.write(data);
