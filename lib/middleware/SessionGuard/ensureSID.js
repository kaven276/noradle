...
module.exports = function ensureSID(req, res, bakRes, rb, cfg){
  ...
  if (!msid || !bsid) {
    ...
    if (!msid) {
      ...
      setCookies.push(fmt('MSID=%s%s;Version=1;Max-Age=%s;Path=/;httpOnly', msid, host ? (';Domain=.' + host) : '', 10000 * 24 * 60 * 60));
    }
    if (!bsid) {
      bsid = guidMgr.create(getStripe());
      setCookies.push(fmt('BSID=%s%s;Version=1;Path=/;httpOnly', bsid, host ? (';Domain=.' + host) : ''));
    }
  }
