...
function releaseCache(){
  ...
  var bucket = LAH[lastClearBucketNo];
  for (n in bucket) {
    var p = n.split('#')
      , prog = p[0]
      , url = p[1]
  ...
  LAH[lastClearBucketNo] = {};
  debug('clear bucket no ', lastClearBucketNo, curBucketNo);
}
...
exports.filter = function(cfg){
  ...
  startTimer(gwcfg.BucketTimespan || 1, gwcfg.BucketAmount || 10);
  setInterval(releaseCache, (gwcfg.RecycleInterval || 1) * 60 * 1000);
  reservedSize = gwcfg.ReservedSize || reservedSize;
