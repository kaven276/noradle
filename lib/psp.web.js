...
function pspdweb(req, res, next){
  ...
  logger.turn('No.%d request %s', ++stat.allCnt, reqUrl);
  ...
  switch (rb.prog) {
    ...
    default:
      stat.reqCnt++;
      normalReq = true;
      DBInMgr.findFreeOraSockThen(reqUrl, function(c, br){
        if (c) {
          oraSock = c;
          busyRec = br;
  ...
  function process_header(data){
    logger.turn('NO.%d first respond', stat.allCnt);
  ...
  function writeToLength(data){
    ...
    logger.turn('NO.%d received chunk %d/%d. cLen=%d', stat.allCnt, data.length, bytesRead, cLen);
    if (bytesRead < cLen) {
      res.write(data);
      md5Content && md5Content.write(data);
