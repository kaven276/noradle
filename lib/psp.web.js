...
function pspdweb(req, res, next){
  ...
  var interrupter = dbm.findFree(reqUrl, null, function(err, oraReq){
    if (err) {
    ...
    if (rb.x$app && rb.c$BSID) {
      session = sessionStore.gets(rb.x$app, rb.c$BSID);
    ...
    function cb(oraRes){
      ...
      // if oracle create new session with unique BSID
      if (headers.s$BSID) {
