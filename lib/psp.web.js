...
ReqBase.prototype.toOraLines = function(){
  var a = this;
  return [a.method, a.url, a.host, a.hostp, a.port, a.base, a.dad, a.prog, a.pack, a.proc, a.path, a.search, a.caddr, a.cport].join(CRLF);
};
ReqBase.parse = function(req, method, url){
  var r = new ReqBase();
  ...
  r.base = cfg.plsql_mount_point.replace(/(^\/|\/$)/, '') || '';
  path_sects.shift();
  r.dad = path_sects.shift();
  r.prog = path_sects.shift() || 'default_b.d';
  try {
  ...
  r.search = url.search ? url.search.substr(1) : '';

  r.url = req.url;
  r.caddr = req.connection.remoteAddress;
  r.cport = req.connection.remotePort;
  return r;
};
...
function pspdweb(req, res){
  ...
  if (url.pathname === '/favicon.ico') {
    var favi = favicon();
    res.writeHead(favi.status, favicon.headers);
    res.end(favi.body);
    return;
  }

  var rb = ReqBase.parse(req, req.method, url);
  ...
  function send2oracle(){
    ...
    var ora_head = [];
    if (reqHeaders['referrer']) {
      reqHeaders['referer'] = reqHeaders['referrer'];
    ...
    if (reqHeaders.cookie) {
      reqHeaders.cookie.split(/[;,] */).forEach(function(cookie){
        var nv = cookie.split('=');
