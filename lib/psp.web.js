...
function pspdweb(req, res, next){
  ...
  var rb = new ReqBaseC(req);
  if (!rb.x$prog) {
    rb = new ReqBase(req, res, next);
  }
  req.rb = rb;
    ...
    default:
      ...
      findFreeOraSockThen(reqUrl, req.connection, function(err, c, seq, cb){
        if (!err) {
          ...
          oraSock.once('response_timeout', onResponseTimeout);
          oraSock.once('socket_released', onSocketReleased);
        } else {
  ...
  function accept_oracle_data(data){
    try {
      ...
      ((follows && follows[0] !== 'head') ? ((cLen && true) ? writeToLength : writeToMarker) : process_header)(data);
    } catch (e) {
  ...
  function process_header(data){
    ...
    bodyChunk.length && ((cLen && true) ? writeToLength : writeToMarker)(bodyChunk);
  }

  function fin(){
    var sect = follows.shift();
    if (sect === 'main') {
      if (md5Content) {
        md5Content.end();
        md5Content = null;
    ...
    if (follows.length === 0) {
      //console.log(res.writable, res.finished, res.statusCode);
  ...
  function writeToMarker(data){
    var bLen = data.length
      ...
      ;
    if (vFin) {
      data = data.slice(0, bLen - endMark.length);
    }
    resWrite(data);
    if (vFin) {
      fin();
  ...
  function writeToLength(data){
    if (bytesRead + data.length > cLen) {
      var rest = cLen - bytesRead
        , extra = data.slice(rest)
        ;
    }
    data = data.slice(0, rest);
