...
function pspdweb(req, res, next){
  ...
  function ensureSID(){
    ...
    function newSession(){
      ...
      uaHash = utl.hash2(req.headers['user-agent']);
    }

    if (!msid && !bsid) {
      // browser first access the site, or browser forbid permanent cookie and start new session
      computeHost();
      msid = guidMgr.create();
      newSession();
      tEnd = new Date(Date.UTC(9999, 1, 1, 0, 0, 0, 0)).toGMTString();
      if (host) {
        res.writeHead(302, {
          'Location' : reqUrl,
          'Set-Cookie' : [
            fmt('MSID=%s;domain=%s;expire=%s;path=/', msid, host, tEnd),
            fmt('BSID=%s;domain=%s;path=/', bsid, host)]
        });
      } else {
        res.writeHead(302, {
          'Location' : reqUrl,
          'Set-Cookie' : [
            fmt('MSID=%s;expire=%s;path=/', msid, tEnd),
            fmt('BSID=%s;path=/', bsid)]
        });
      }
      res.end();
      return;
    ...
    if (!bsid) {
      ...
      newSession();
      if (host) {
        ohdr['Set-Cookie'] = fmt('BSID=%s;Domain=%s;path=/', bsid, host);
      } else {
        ohdr['Set-Cookie'] = fmt('BSID=%s;path=/', bsid);
      }
      // todo: if oracle's Set-Cookie will override this Set-Cookie
    }
    if (!uaHash) {
      uaHash = utl.hash2(req.headers['user-agent']);
      delete req.headers['user-agent'];
    }
    rb.setSid(base64alt(bsid), base64alt(msid || ''), uaHash || '');
  }
