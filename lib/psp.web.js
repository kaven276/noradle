...
function pspdweb(req, res, next){
  ...
  function ensureSID(){
    var host, tEnd
      ...
      , uaHash = utl.hash2(ua)
      ;
    if (ua.match(cfg.NoneBrowserPattern)) return;
    ...
    console.log(req.socket.remoteAddress + '.' + req.socket.remotePort, fstReq);
    ...
    if (!msid && fstReq) {
      ...
      // Notice: set Content-Length to 0 is fatal, so browser will use keep-live
      if (host) {
        res.writeHead(302, {
          'Content-Length' : '0',
          'Set-Cookie' : [
