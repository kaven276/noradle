...
function pspdweb(req, res, next){
  ...
  logger.turn('No.%d request %s', reqNo, reqUrl);
  ...
  function process_header(data){
    logger.turn('NO.%d first respond', reqNo);
  ...
  function writeToLength(data){
    ...
    logger.turn('NO.%d received chunk %d/%d. cLen=%d', reqNo, data.length, bytesRead, cLen);
    if (bytesRead < cLen) {
      res.write(data);
      md5Content && md5Content.write(data);
