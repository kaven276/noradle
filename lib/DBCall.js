var bUnitTest = (process.argv[1] === __filename)
  ...
  , connMonitor = require('./db.js').connectionMonitor
  , CRLF = '\r\n'
...
DBCall.prototype.call = function(prog, params, cb){
  ...
  DBInMgr.findFreeOraSockThen(function(oraSock){
    callOra.call(self, dbName, dbuName.toLowerCase(), spName.toLowerCase(), params, {}, cb, msgEmitter, oraSock);
  });
  return msgEmitter;
};
 ...
 * @return {*}
 */
function callOra(db, dbu, prog, params, headers, cb, msgEmitter, oraSock){
  var self = this
    , parts = prog.split('.')
    , oraStatus
    , oraResHead2
  ...
  function acceptOracleResult(data){
    ...
    if (!oraResHead2) {
      ...
      if (bMsgStream) {
        msgEmitter.emit('start', oraStatus, oraResHead2);
...
// Unit Test
connMonitor.once('connect', function(){
  if (!bUnitTest) return;
