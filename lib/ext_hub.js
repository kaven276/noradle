...
/**
 * pre-connect to all worker proxy at startup
 */
require('../worker_proxy_cfg.js').forEach(function(wp, id){
  if (!wp) return;
  var hostp = wp[0].split(':')
    , wpw = wpSocks[id] = new WPWrapper(new net.Socket({allowHalfOpen : true}), id, hostp, wp[2], wp[1])
    ;
  wpw.connect();
});
...
server.listen(parseInt(process.argv[2]) || 1523);